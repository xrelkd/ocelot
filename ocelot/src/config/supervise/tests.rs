...
type: sigterm
";
    let config: ShutdownSignalConfig = serde_yaml::from_slice(yaml).unwrap();
...
fn test_restart_policy_never() {
    let yaml = b"type: Never";
    let policy: RestartPolicyConfig = serde_yaml::from_slice(yaml).unwrap();
...
processes: {}
";
    let config: SuperviseConfig = serde_yaml::from_slice(yaml).unwrap();
...
fn test_probe_config_default_handler() {
    let yaml = b"{}";
    let config: ProbeConfig = serde_yaml::from_slice(yaml).unwrap();
...
condition: Healthy
";
    let config: DependencyConfig = serde_yaml::from_slice(yaml).unwrap();
...
#[test]
fn test_supervise_config_template_minimal() {
    let template = SuperviseConfig::template_minimal();
    let config: SuperviseConfig = serde_yaml::from_slice(&template).unwrap();
    assert_eq!(config.processes.len(), 1);
    assert!(config.processes.contains_key("http-server"));
}

#[test]
fn test_supervise_config_template_basic() {
    let template = SuperviseConfig::template_basic();
    let config: SuperviseConfig = serde_yaml::from_slice(&template).unwrap();
    assert!(config.processes.len() >= 2);
}

#[test]
fn test_supervise_config_template_full() {
